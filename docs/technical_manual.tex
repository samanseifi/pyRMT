...
The time step obeys the advective, viscous, and elastic-wave CFL limits,
\(
\Delta t \le \min\!\bigl(C\,\Delta x/U,\;
C_\nu\,\Delta x^2/\nu,\;
C\,\Delta x/c_s\bigr),
\)
with $c_s=\sqrt{\mu_s/\rho}$.
...
\begin{itemize}[nosep]
  ...
  \item Explicit time stepping ties $\Delta t$ to the elastic-wave / viscous CFL.
...
\texttt{pyRMT/mac.py} & staggered grid, exact projection, momentum predictors, \\
 & periodic/Neumann Poisson, interfacial \& contact stress \\
